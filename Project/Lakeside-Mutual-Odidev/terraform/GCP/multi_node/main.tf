...
resource "google_compute_instance" "lakeside" {
  project                   = var.project
  name                      = "lakeside"
  machine_type              = var.lakeside_master_node_machine_type
  zone                      = var.zone
  allow_stopping_for_update = true
...
resource "google_compute_instance" "lakeside-node" {
  project                   = var.project
  count                     = var.lakesideNodeCount
  name                      = "lakeside-node-${count.index + 1}"
  machine_type              = var.lakeside_worker_node_machine_type
  zone                      = var.zone
  allow_stopping_for_update = true
  tags                      = ["private", "http-server", "https-server"]
  boot_disk {
